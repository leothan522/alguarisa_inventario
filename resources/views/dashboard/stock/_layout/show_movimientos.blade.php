    ...
    <div class="col-md-12" xmlns:wire="http://www.w3.org/1999/xhtml">
        <div class="card">
            ...
            <div class="card-header border-0">
                <h3 class="card-title text-uppercase"><small>{{ $almacen->nombre }}</small></h3>
                <div class="card-tools">
                    <button type="button" class="btn btn-tool" wire:click="setLimit"
                            @if($rows > $rowsMovimientos) disabled @endif >
                        <i class="fas fa-sort-amount-down-alt"></i> Ver más
                    </button>
                    <a href="{{ route('movimientos.reportes', [$almacenes_id, $empresas_id, $rowsMovimientos]) }}"
                       class="btn btn-tool btn-sm" target="_blank">
                        <i class="fas fa-download"></i>
                    </a>
                    <button type="button" class="btn btn-tool" data-card-widget="remove"
                            wire:click="limpiarStock" {{--onclick="verSpinnerOculto()"--}}>
            ...
            <div class="card-body table-responsive p-0" @if($tableStyle) style="height: 60vh;" @endif>
                <table class="table table-head-fixed table-striped table-valign-middle table-sm">
                    ...
                    <tr>
                        <th>Tipo</th>
                        <th class="d-none d-lg-table-cell">Modulo</th>
                        <th class="d-none d-md-table-cell">Código</th>
                        <th class="d-none d-md-table-cell">Fecha</th>
                        <th class="d-none d-lg-table-cell">Articulo</th>
                        <th>Descripción</th>
                        {{--<th>Segmento</th>--}}
                    ...
                    @foreach($listarMovimientos as $movimientos)
                        @foreach($movimientos['detalles'] as $detalle)
                            <tr>
                                <td>{{ $detalle['tipo'] }}</td>
                                <td class="d-none d-lg-table-cell text-uppercase">{{ $movimientos['tabla'] }}</td>
                                <td class="d-none d-md-table-cell text-uppercase">{{ $movimientos['codigo'] }}</td>
                                <td class="d-none d-md-table-cell">
                                    {{ getFecha($movimientos['fecha'], 'd/m/Y h:i:s a') }}
                                </td>
                                <td class="d-none d-lg-table-cell">
                                    <span class="text-uppercase">{{ $detalle['codigo'] }}</span>
                                </td>
                                <td class="text-uppercase"><span>{{ $detalle['articulo'] }}</span></td>
